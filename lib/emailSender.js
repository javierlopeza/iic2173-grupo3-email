...
exports.sendEmail = function (receiver, name, subject, data) {
  ...
  var mailOptions = {
    ...
    html: buildHTML({
      ...
      purchaseRequested: data.purchaseRequested,
...
function buildHTML(data) {
  ...
  if(data.purchaseRequested) {
    addressMessage = data.validAddress ? `<br><h3>Sus compras aceptadas serán despachadas a: ${data.deliveryAddress}</h3>` : "<br><h3>Debe ingresar una dirección de despacho para poder realizar compras!</h3>"
  }  
  ...
  return mainTitle + productsTable + acceptedPurchasesTable + rejectedPurchasesTable + addressMessage
}
